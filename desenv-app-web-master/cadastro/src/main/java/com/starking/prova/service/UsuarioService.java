...
public class UsuarioService {
	...
	public List<UsuarioResponse> listar() {
		return (List<UsuarioResponse>) converterUtils.convertToListResponse(this.repository.findAll(),
				UsuarioResponse.class);

	}

	public void salvarUsuario(UsuarioRequest request) {
		this.repository.save((Usuario) converterUtils.convertRequestToEntity(request, Usuario.class));
	}

	public UsuarioResponse buscarPorId(Long id) {
		return (UsuarioResponse) converterUtils.convertEntityToResponse(
	...
	public void updateUsuario(UsuarioRequest request, Long id) {
		var entity = this.repository.findById(id).orElseThrow(() -> new EntityNotFoundException("Entity not found"));
	}
